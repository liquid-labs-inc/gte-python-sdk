...
from gte_py.models import OrderStatus
...
async def show_order_book(client, market_address):
    ...
    try:
        ...
        for i, ask in enumerate(reversed(book['asks']), 1):
            print(f"  {i}. Price: {ask['price']} | Size: {ask['size']} | Orders: {ask['orderCount']}")
            
        print("\nBids (Buy Orders):")
        for i, bid in enumerate(book['bids'], 1):
            print(f"  {i}. Price: {bid['price']} | Size: {bid['size']} | Orders: {bid['orderCount']}")
