...
async def main() -> None:
    ...
    open_orders = await paged_request(
        lambda limit, offset: client.user.get_filled_orders(market, limit=limit, offset=offset), 50, 1000)
    print("Opening orders:")
    for open_order in open_orders:
        print(
            f"Order ID: {open_order.order_id}, Price: {open_order.price}, Amount: {open_order.filled_amount}, Side: {open_order.side}")
