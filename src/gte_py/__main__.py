...
@app.command()
def assets(creator: str | None = None, limit: int = 100, offset: int = 0):
    """Get list of assets."""
...
@app.command()
def candles(market_address: str, interval: str = "1h", days: int = 1, limit: int = 500):
...
def watch_market(
    ...
    async def _watch_market():
        async with Client() as client:
            ...
            def on_trade(trade):
                typer.echo(
                    f"[TRADE] {trade.datetime.isoformat()} | {trade.side.upper()} {trade.size} @ {trade.price}"
                )

            def on_candle(candle):
                typer.echo(
                    f"[CANDLE] {candle.datetime.isoformat()} | O:{candle.open} H:{candle.high} L:{candle.low} C:{candle.close} V:{candle.volume}"
                )
            ...
            def on_orderbook(update):
                ...
                typer.echo(
                    f"[ORDERBOOK] {update.datetime.isoformat()} | Bid: {best_bid} | Ask: {best_ask} | Spread: {update.spread}"
                )
...
def user_positions(user_address: str):
    ...
    async def _user_positions():
        ...
        async with Client(w3, user_address) as client:
            ...
            positions_data = [
                {
                    "market": {"address": pos.market.address, "pair": pos.market.pair},
            ...
            typer.echo(
                json.dumps({"positions": positions_data, "total": len(positions)}, indent=2)
            )
