...
class EventStream(Generic[T]):
    ...
    def create_filter(self, **filter_params) -> "EventStream[T]":
        ...
        self.filter = self.event.createFilter(
            fromBlock=self.from_block, toBlock=self.to_block, **filter_params
        )
        return self
    ...
    def process_events(
        self, handler: Callable[[T], None], exit_condition: Optional[Callable[[], bool]] = None
    ):
        """
...
class CLOBEventStreamer:
    ...
    def __init__(self, clob: ICLOB, from_block: int = 0, poll_interval: float = 2.0):
        ...
        self._callbacks = {"orderbook": [], "tob": [], "trades": [], "stats": []}

    # ================= EVENT STREAMS =================

    def limit_order_processed_stream(
        self, **filter_params
    ...
    def get_order_book_snapshot(
        self, depth: int = 5, side: Optional[int] = None
    ) -> OrderBookSnapshot:
        """
        ...
        if side is None or side == 0:  # Include bids
            ...
            for _ in range(depth):
                ...
                (num_orders, head_order_id, tail_order_id) = self.clob.get_limit(
                    current_price, 0
                )  # 0 is BUY side
    ...
    def stream_order_book(
        self, depth: int = 5, callback: Optional[Callable] = None
    ...
    def stream_trades(self, callback: Optional[Callable] = None) -> Iterator[Dict[str, Any]]:
        ...
        order_matched_filter = self.contract.events.OrderMatched.createFilter(fromBlock="latest")
